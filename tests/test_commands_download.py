...
def test_download_emojis(cli_runner, tmp_path):
    ...
    for e in emoji_names:
        emoji_filenames.append("{}.png".format(e))
        emoji_sha1s.append(EMOJIS[e]["sha1"])
    user = "user-1"
    # Test
    ...
    for i, path in enumerate(paths):
        ...
        with (destination / emoji_filenames[i]).open("rb") as f:
            assert hashlib.sha1(f.read()).hexdigest() == emoji_sha1s[i]
...
def test_no_clobber_download(cli_runner, tmp_path):
    ...
    destination = tmp_path
    emoji_names = ["emoji_1", "emoji_2"]
    emoji_filenames = ["{}.png".format(e) for e in emoji_names]
    emoji_sha1 = EMOJIS[emoji_names[1]]["sha1"]
    user = "user-1"
    # Test
